...
def render_tab_tracker():
    ...
    # ── 🎤 유튜버 타이밍 알림 (RAG 영상에서 시점 기반 알림 자동 추출) ──
    try:
        ...
        # 자동 갱신: 12시간 이상 묵었으면 페이지 진입 시 한 번만 실행
        if _needs_refresh(stale_hours=12) and not st.session_state.get("_video_refresh_attempted"):
        ...
        if active_alerts:
            ...
            st.markdown(
                ...
                f"(RAG 영상 분석 · 12시간마다 자동 갱신 · 마지막 {gen_label})</span></div>"
                f"{cards}"
                f"</div>",
                unsafe_allow_html=True,
            )
        # 알림 0개인 경우엔 굳이 표시 안 함 (지침성 정보가 없으면 깔끔하게)
    except Exception as e:
